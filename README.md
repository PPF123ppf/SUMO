...
基于 SUMO 高速公路事故场景的全栈交通仿真系统，实现 **Stackelberg 混合博弈换道决策 + 8 维原子特征空间 + 最大熵逆强化学习 (MaxEnt IRL)**，支持 0%~100% CAV 渗透率的混合交通实验，通过 4 种基线模型的逐层消融实验量化各功能模块的贡献。
...
实验框架通过逐层关闭功能模块，量化每个模块的贡献。四个模型构成三层消融：
...
**实验矩阵**：4 模型 × 4 密度 (1200/2000/2800/3600 pcu/h) × 6 渗透率 (0%/10%/30%/50%/70%/100%) + 额外的纯人类驾驶对照 (0% CAV) = 96+ 组，每组单次仿真 360s，8 核并行执行。
...
| 模型 | 1200 | 2000 | 2800 | 3600 | 碰撞 | 平均队列 |
|------|:----:|:----:|:----:|:----:|:--:|:--:|
| **Game (Ours)** | **77** | **126** | **173** | **204** | **0** | 18 |
| No-V2X | 77 | 126 | 173 | 203 | **2** | 21 |
| Rule-Based | 48 | 80 | 106 | 135 | 0 | 38 |
| SUMO Default | 61 | 35 | 41 | 57 | 2 | 48 |

### 14.3 消融层分析

**L1 (Game vs No-V2X) — V2X 通信的贡献**：通过量完全一致（Game 77/126/173/204，No-V2X 77/126/173/203），但 No-V2X 发生 2 次碰撞，Game 零碰撞。V2X 通信在这个场景下的贡献是**安全而非效率**——协同让行和全局广播防止临界条件下的追尾碰撞。

**L2 (No-V2X vs Rule-Based) — 博弈决策的贡献**：Rule-Based 的通过量在 2000pcu/h 时下降 36%（126→80），在 3600pcu/h 时下降 33%。博弈模型在相同无通信条件下，通过更丰富的决策空间（混合博弈结构、自适应权重、社会冲击）产生了显著更好的效果。

**L3 (Rule-Based vs SUMO Default) — Python TraCI 控制的贡献**：SUMO Default 在 2000pcu/h 仅通过 35 辆——不到 Game 的 30%，且发生 2 次碰撞。SL2015 子车道换道虽然在物理层面精细，但缺乏对事故场景的适应能力。打开 Python 控制后，即使只是多条件规则（Rule-Based），通过量也提升了 2 倍以上。

---

## 15. 核心结论

1. **Game 模型在所有维度全面领先** — 2000pcu/h 时通过 126 辆 (SUMO Default 仅 35 辆，3.6 倍差距)，零碰撞，队列最短
2. **CAV 渗透率从 10% 起不影响通过量** — 0%→10%→100% 各渗透率下通过量无趋势性变化 (77-79 辆，1200pcu/h)；30% CAV 即获得全部博弈收益，无需等待高渗透率
3. **V2X 通信的贡献在安全层面，非效率层面** — Game vs No-V2X 通过量一致，但 No-V2X 发生碰撞而 Game 零碰撞。协同让行是零碰撞的机制保障
4. **博弈决策比规则判断提升 36% 的效率** — No-V2X vs Rule-Based 在 2000pcu/h 的差距从 126 降至 80（-36%），博弈决策保留了全部效率增益
5. **数据驱动的 IRL 权重无需人工调参** — AD4CHE 数据集训练的权重 (lc_cost=0.70, safe=0.25, speed=0.15) 与人类驾驶行为一致：真人天生不爱换道，安全是普遍考虑，速度驱动力被高估
6. **逐层消融验证了三个模块的各自贡献** — V2X→安全、博弈→效率、Python 控制→全部。关掉所有三层后（SUMO Default），效率降至不到 30%
...
├── baseline_comparison.py    # 4 种基线模型 (SUMO Default, Rule-Based, No-V2X)
