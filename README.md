...
| **100%** | **218 / 176 / 12.1s / 23 / 0.7** | 205 / 79 / 14.7s / 41 / 2.0 | 213 / 133 / 12.3s / 37 / 0.7 | 56 / 0 / 34.5s / 92 / 0.0 | 49 / 54 / 3.0s / 17 / 0.7 |
...
|:------|:--------:|:----:|:----:|:----:|:--:|:--------:|
| **Game (Ours)** | **218** | **12.1s** | **176** | **23** | 0.7 | **0%** |
| No-V2X | 213 | 12.3s | 133 | 37 | 0.7 | **0%** |
| DRL (PPO) | 205 | 14.7s | 79 | 41 | 2.0 | **0%** |
| Rule-Based | 49 | 3.0s | 54 | 17 | 0.7 | 1% |
| SUMO Default | 56 | 34.5s | 0 | 92 | 0.0 | **57%** |

### 14.3 消融层分析

**渗透率趋势**：3 轮平均后 Game 在 0%-100% CAV 间通过量稳定在 209-223 辆（波动 <7%），100% 时最优（218 辆、12.1s 延误、23 队列）。DRL 在低渗透率下与 Game 接近，70%-100% 时队列差距明显（41 vs 33/23）。

**L1 (Game vs No-V2X) — V2X 通信的贡献**：两者差距很小（218 vs 213，+2%），全 CAV 环境下每辆车独立博弈也能做出相似决策。V2X 在本文设定下效率增益有限，但提供了协同让行的安全冗余。

**L2 (No-V2X vs Rule-Based) — 博弈决策的贡献**：Game 通过 218 辆，远高于 Rule-Based 的 49 辆（+345%）。即使无 V2X 通信，博弈模型的决策空间也带来质的提升。

**L3 (Rule-Based vs SUMO Default) — Python TraCI 控制的贡献**：SUMO Default 仅通过 56 辆，Jerk 违例率 57%。

**DRL vs Game — 博弈 vs 强化学习的对比**：3 轮平均后 DRL 与 Game 通过量差距缩小到 6%（205 vs 218）。但 DRL 碰撞率 2.0 高于 Game 的 0.7，队列更长（41 vs 23），表明 PPO 在安全性和交通流协调方面仍不及博弈方法。

---

## 15. 核心结论

1. **Game 模型在 6 档渗透率下全面领先（3 轮平均）** — 3600pcu/h 下通过量 209-223 辆，波动仅 7%（0%→100%），100% 时最优（218 辆、12.1s 延误、23 队列）
2. **CAV 渗透率不影响 Game 的通过量** — 10%-100% 间通过量稳定在 209-218 辆，更多 CAV 不提升通过量但换道次数从 11 增至 176
3. **V2X 通信的效率增益有限** — Game vs No-V2X 在全 CAV 下差距仅 +2%（218 vs 213）
4. **博弈决策比规则判断提升 345% 的效率** — Game 通过 218 辆 vs Rule-Based 仅 49 辆
5. **数据驱动的 IRL 权重无需人工调参** — AD4CHE 数据集训练的权重与人类驾驶行为一致
6. **逐层消融验证了三个模块的各自贡献** — 博弈→效率、V2X→安全、TraCI→基础控制
7. **DRL（PPO）通过量接近 Game（205 vs 218，-6%），但碰撞（2.0 vs 0.7）和队列（41 vs 23）明显更高** — 消除随机性后 DRL 的效率差距小于单轮结果，但安全与协调差距依然显著
