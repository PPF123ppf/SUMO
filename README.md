...
1. **Game 模型全面领先** — 通过量最高、零碰撞。2000pcu/h 时 Game 126 辆，SUMO Default 仅 35 辆，差距 3.6 倍
2. **渗透率不影响通过量** — 从 0% 到 100%，通过量基本不变（77~79 辆，1200pcu/h），30% CAV 已足够获得全部收益
3. **V2X 通信贡献在安全，非效率** — Game vs No-V2X 通过量完全一致，但 No-V2X 有 2 次碰撞，Game 零碰撞。V2X 协同让行是安全保障
4. **数据驱动的权重** — 所有博弈权重通过 MaxEnt IRL 从 AD4CHE 数据集学习，非人工调参
5. **SUMO Default 高密度崩溃** — 2800pcu/h 出现碰撞，2000~3600pcu/h 通过量仅为 Game 的 27~34%
6. **Rule-Based 改进后效果可接受** — 多条件规则（TTC 前向 1.5s + 后向 1.0s）产生实际换道活动，位于 Game 和 SUMO Default 之间
